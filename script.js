...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderTasks() {
    ...
    if (filtered.length === 0) {
      ...
      empty.innerHTML = `<span>
          ...
          <small>Add a reminder above.</small>
        </span>`;
  ...
  taskList.addEventListener("touchmove", (event) => {
    const li = event.target.closest(".task-item");
    if (!li) return;
    const moveX = event.touches[0].clientX - Number(li.dataset.startX);
    if (moveX < 0) {
  ...
  taskList.addEventListener("touchend", (event) => {
    const li = event.target.closest(".task-item");
    if (!li) return;
    const moveX = event.changedTouches[0].clientX - Number(li.dataset.startX);
    if (moveX < -100) {
